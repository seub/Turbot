...
GameZ::GameZ(const PlayerZ * const whitePlayer, const PlayerZ * const blackPlayer) : currentPosition(true), whitePlayer(whitePlayer), blackPlayer(blackPlayer)
{
    moves.clear();
    movePGNs.clear();
    moveTimes.clear();
...
bool GameZ::isFinished() const
{
    return (result!=GameResult::NOT_FINISHED);
}

void GameZ::playMove(const MoveZ &move, const bool &claimDraw, std::chrono::duration<double> moveTime)
{
    if (isFinished()) {throw("Game is finished!");}
    ...
    {
        PositionZ newPosition(false);
        LegalMoverZ currentMover(&currentPosition, true);

        currentMover.applyMove(move, newPosition, true);

        moves.push_back(move);
        moveTimes.push_back(moveTime);
        movePGNs.push_back(MovePGNZ(move, &currentMover));
        turn = !turn;

        LegalMoverZ newMover(&newPosition, true);
...
std::string GameZ::printTagRoster() const
{
    std::string out = {};
...
std::string GameZ::printResult() const
{
    std::string out = {};
    switch(result)
    {
...
std::string GameZ::printPGN(bool printTagRoster) const
{
    std::string out = {};
...
bool GameZ::exportPGN(bool printTagRoster) const
{
    std::ofstream file(createFilename());
    if (file.is_open())
    {
...
void GameZ::play()
{
    ...
    while (!isFinished())
    {
        ...
        const PlayerZ *nextPlayer = turn ? whitePlayer : blackPlayer;
        bool claimDraw;
        ...
        {
            LegalMoverZ mover(&position, true);
            message += " played ";
            message += MovePGNZ(move, &mover).toPGN(position.getMoveNumber());
            message += ".";
        }
        message += " [Move time: ";
        message += Tools::convertDoubleToString(moveTime.count());
...
std::string GameZ::createFilename() const
{
    std::string res = {};
    res += "Turbot_";
    res += date;
...
void GameZ::averageMoveTime(std::chrono::duration<double> &white, std::chrono::duration<double> &black) const
{
    uint nbWhite =0 , nbBlack = 0;
    white = std::chrono::duration<double>::zero();
    black = std::chrono::duration<double>::zero();
